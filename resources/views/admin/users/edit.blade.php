...
<div class="form-container" style="background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(20px); border-radius: 20px; padding: 2rem; box-shadow: 0 8px 32px rgba(31, 38, 135, 0.37); border: 1px solid rgba(255, 255, 255, 0.2);">
    <form action="{{ route('users.update', $user->id) }}" method="POST">
        ...
        {{-- <div style="margin-bottom: 1.5rem;">
            <label for="daily_operations" style="display: block; font-weight: bold; margin-bottom: 0.5rem; color: #333;">العمليات اليومية</label>
            <input type="number" id="daily_operations" name="daily_operations" value="{{ old('daily_operations', $user->daily_operations) }}" class="form-control" style="width: 100%; padding: 0.75rem; border: 1px solid #ccc; border-radius: 8px; box-sizing: border-box;">
        </div>
